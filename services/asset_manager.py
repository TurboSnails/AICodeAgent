...
from utils.config_loader import cfg_str, cfg_float
...
def _figma_api_request(req: urllib.request.Request, timeout: int = 60, max_retries: int = 3) -> Optional[bytes]:
    ...
    base_delay = cfg_float("figma.retry_delay", 2.0)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if e.code == 429:
                ...
                logger.warning(f"429 Rate Limited, retrying in {delay}s (attempt {attempt + 1}/{max_retries + 1})")
                time.sleep(delay)
                continue
            if 500 <= e.code < 600:
                delay = (2 ** attempt) * base_delay
                logger.warning(f"{e.code} Server Error, retrying in {delay}s (attempt {attempt + 1}/{max_retries + 1})")
                time.sleep(delay)
                continue
            ...
            last_err = e
            delay = (2 ** attempt) * base_delay
            logger.warning(f"{e}, retrying in {delay}s (attempt {attempt + 1}/{max_retries + 1})")
            time.sleep(delay)
    logger.error(f"Max retries exceeded: {last_err}")
    return None
...
def _similarity_threshold() -> float:
    return cfg_float("figma.hash_similarity", 0.95)
...
def find_reusable_local(figma_bytes: bytes, local_map: Dict[str, Path], figma_name: str = "") -> Optional[Tuple[str, str]]:
    ...
    for stem, path in local_map.items():
        ...
        if figma_name:
            ...
            if sim >= _similarity_threshold() and (best_match is None or sim > best_match[1]):
...
def fetch_figma_svg_via_api(
    ...
    for node_id, img_url in images.items():
        ...
        if svg_data is None:
            logger.warning(f"download {node_id}: failed after retries")
            continue
        try:
            ...
            logger.error(f"write {node_id}: {e}")
...
def process_visual_assets(
    ...
    if not file_key and meta_path.exists():
        try:
            ...
            pass
    if not file_key:
        file_key = cfg_str("figma.file_key", "")
    ...
    if token and file_key:
        ...
        if node_ids:
            logger.info(f"Figma API 嗅探到 {len(node_ids)} 个候选节点")
    ...
    logger.info(f"asset_map 更新完成，共 {len(asset_map.get('assets', []))} 条")
    return asset_map
