...
def transition(task_id: str, to_state: State, reason: str = "") -> bool:
    ...
    try:
        ...
        if to_state not in VALID_TRANSITIONS.get(from_state, []):
            print(f"[STATE ERROR] 非法状态流转: {from_state.value} -> {to_state.value}")
            conn.execute("ROLLBACK")
            conn.close()
            return False
        ...
        conn.commit()
        print(f"[STATE] {task_id}: {from_state.value} -> {to_state.value} | {reason}")
        return True
...
def approve_gate_resume(task_id: str) -> bool:
    ...
    try:
        ...
        conn.commit()
        print(f"[STATE] {task_id}: {from_state} -> pending (resume_from_gate=1)")
        return True
    except sqlite3.OperationalError as e:
        print(f"[STATE ERROR] SQLite operational error: {e}")
        conn.execute("ROLLBACK")
