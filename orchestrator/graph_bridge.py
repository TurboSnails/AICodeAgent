...
CRG_HTTP = os.environ.get("CRG_HTTP_URL", "http://127.0.0.1:5555")
CRG_REPO = os.environ.get("CRG_REPO_ROOT", str(PROJECT_ROOT))
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            if "result" in body:
                return body["result"]
            if "error" in body:
                print(f"[CRG HTTP] {body['error']}")
    except (urllib.error.URLError, TimeoutError, json.JSONDecodeError) as e:
        print(f"[CRG HTTP] unavailable: {e}")
    return None
...
def crg_status() -> bool:
    """检查本地 CRG HTTP 是否可用"""
    try:
        req = urllib.request.Request(CRG_HTTP.rstrip("/") + "/health", method="GET")
...
def ensure_crg_http_background() -> bool:
    ...
    if crg_status():
        return True
    if os.environ.get("CRG_AUTO_START", "0") != "1":
        return False
    port = CRG_HTTP.split(":")[-1].rstrip("/")
    ...
    try:
        with open(log, "a", encoding="utf-8") as lf:
            subprocess.Popen(
                cmd, stdout=lf, stderr=lf, cwd=CRG_REPO,
                start_new_session=True,
            )
        return True
    except Exception as e:
        print(f"[CRG] auto-start failed: {e}")
        return False
