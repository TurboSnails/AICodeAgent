...
│   ├── notifying.py              # 通知分发
│   ├── _review_utils.py          # 审查阶段共享工具（prompt 构建/verdict 解析）
│   └── _fix_plan.py              # 结构化修复计划（FixPlan/FixItem/FixPriority）
│
├── services/
...
│   ├── tencent_memory_service.py # TencentDB Agent Memory 集成
...
│   ├── paths.py                  # 路径常量
│   ├── phase_status.py           # 阶段进度写入（phase_status.json 可观测性）
│   ├── memory_context.py         # TencentDB 记忆召回注入上下文
│   ├── project_guides.py         # 项目规范文档注入（dev_protocol 等）
│   ├── tracing.py                # 任务执行链路追踪
│   ├── task_cancel.py            # 任务取消信号检测
│   ├── cli_progress.py           # Claude CLI 流式进度解析
│   └── request_logger.py         # AI 调用请求/响应日志
│
├── orchestrator/
...
│           └── test_tencent_memory_service.py
│
├── data/                         # 运行时（gitignore）
├── dist/                         # 构建产物（gitignore）
└── workspace/{task_id}/          # 任务沙箱（gitignore）
    ...
    ├── consensus.md
    ├── plan.md                   # architect_planning 输出的实现计划
    ├── design.md                 # architect_planning 输出的架构设计
    ├── uncertainty_check.json    # architect_planning 不确定性检测结果
    ├── codex_review.md
    ├── red_team_audit.md
    ├── requirement_review.md
    ├── fix_plan.json             # 合并后的结构化修复计划（FixPlan）
    ├── phase_status.json         # 实时阶段进度（可观测性）
    ├── memory_recall.md          # TencentDB 召回内容
    ├── asset_map.json
    ├── escape_log.md
    └── ...
```
...
10. **结构化修复计划**：Review 阶段 FAIL 时，`_fix_plan.py` 从报告文本自动提取 `FixPlan`（优先解析 JSON block，回退内联/章节式 Markdown）。优先级 `CRITICAL > HIGH > MEDIUM > LOW`；`correcting.py` 读取 `fix_plan.json` 生成针对性 fix prompt，加速收敛。
11. **多阶段审查链**：`building` 全绿后依次经过 `self_review`（自查）→ `codex_review`（逻辑/回归）→ `architect_review`（设计质量，可关闭）→ `red_team_review`（攻击视角，L2 默认开启）→ `requirement_review`（需求对齐），任一 FAIL 均回到 `correcting`，修复后从第一个审查阶段重新开始。
...
- [x] Self Review 构建后自检（`self_review.py`：编码产物自查，FAIL 回 correcting）
- [x] Architect Review 架构评审（`architect_review.py`：设计质量审查，可选）
- [x] Architect Planning 规划阶段（`architect_planning.py`：需求拆解 + 不确定性检测，L0 默认跳过）
- [x] 结构化修复计划（`_fix_plan.py`：FixPlan / FixItem / FixPriority，Review 阶段 FAIL 时自动生成）
- [x] TencentDB Agent Memory 长期记忆（`tencent_memory_service.py`：recall/capture/session）
- [ ] Maestro 流程测试
